...
public class ClientWindow extends JFrame {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (!jtfMessage.getText().trim().isEmpty() && (!clientName.isEmpty() && flag)) {//flag = true if sms was written once -> sms != введите sms
                        ...
                        if(kriptoAlgorithms.isDigit(message)) {
                            sendCodeX(".X1.", secretC, jtfMessage.getText());// send X1
                            flag = true;
   ...
   // в отдельном потоке начинаем работу с сервером
                            ...
                            if (inMessage.hasNext()) {
                                ...
                                else if (inMes.equalsIgnoreCase("##ping##"))
                                {
                                    ...
                                    outMessage.flush();
                                           ...
                                           if(inMes.startsWith("."))
                                           {
                                               String temp = inMes.substring(4, inMes.length());
                                               String nameFrom = temp.substring(0, temp.lastIndexOf(":"));
                                               String msg = temp.substring(temp.lastIndexOf(":") + 1, temp.length());
                                               System.out.println("have msg:" + msg);
                                               if(! nameFrom.equalsIgnoreCase(clientName)) {
                                                   if (inMes.startsWith(".X1.")) // its Bob
                                                   {
                                                       sendCodeX(".X2.", secretC, msg);

                                                   }
                                                   else
                                                   if (inMes.startsWith(".X2.")) // its Alice
                                                   {
                                                       sendCodeX(".X3.", secretD, msg);
                                                   }
                                                   else
                                                   if (inMes.startsWith(".X3.")) // its Bob
                                                   {
                                                       Long x3 = Long.parseLong(msg);
                                                       Long x4 = kriptoAlgorithms.degreeByMod(x3, secretD, KriptoAlgorithms.p); // message
                                                       String decodedMessage = nameFrom + ":" + x4.toString();

                                                       jtaTextAreaMessage.append(decodedMessage);
                                                       // добавляем строку перехода
                                                       jtaTextAreaMessage.append("\n");
                                                   }
    ...
    // отправка сообщения
    public void sendCodeX(String prefix, Long degree, String msg)
    {
        ...
        String secretMessage = codeMessage.toString();// kriptoAlgorithms.code(message, secretKeys.get(key));// for himself?
            ...
            String messageStr = prefix + clientName + ":" + secretMessage;
