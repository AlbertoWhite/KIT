        ...
        try {
            ...
            while (true) {
                ...
                if (inMessage.hasNext()) {
                    ...
                    {
                        flIsSystemMessage = true;
                        String name = clientMessage.substring(0, clientMessage.lastIndexOf(":"));
                        Integer key = Integer.parseInt(clientMessage.substring(clientMessage.lastIndexOf(":") + 1, clientMessage.length()));
                        server.setPublicKey(name, key);
                        flagKey = false;
                    }
                    ...
                    if (clientMessage.equalsIgnoreCase("##getlist##messages##"))
                    {
                        flIsSystemMessage = true;
                        sendMsg("History:");
                        for (String msg : clientsMessages) {
                            sendMsg(msg);
                        }
                    }
                    ...
                    if (clientMessage.equalsIgnoreCase("##getlist##keys##")) {
                        flIsSystemMessage = true;

                        server.sendMessageToAllClients("Public keys:");

                        for (String nameUs : clientsNames) {

                            server.sendMessageToAllClients(nameUs + ":" + server.getPublicKey(nameUs));
                        }
                        server.sendMessageToAllClients("##finish##keys##");
                    }
                    if (clientMessage.equalsIgnoreCase("##public##key##")) {
                        flIsSystemMessage = true;
                        flagKey = true;
                     }
